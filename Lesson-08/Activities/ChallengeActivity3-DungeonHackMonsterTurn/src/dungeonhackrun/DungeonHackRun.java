...
public class DungeonHackRun 
{
    ...
    {
        Weapon s = new Weapon("sword","slashes",10);
        Weapon c = new Weapon("arm","claws",6);
        Actor player = new Actor("Sir Santos",5,10,s);
        Actor monster = new Actor("Troll",5,8,c);
        Die d20 = new Die(20);
        ...
        if (!monster.isDead())
        {
            ...
            System.out.printf("%s %s its %s at %s and ", monster.name(), monster.weapon().action(), monster.weapon().name(), player.name());
            if (roll >= monster.armorClass()) 
            {
