...
public class StudentFileIORun {
    ...
    public static void main(String[] args) throws IOException, FileNotFoundException, ClassNotFoundException {
        ...
        File f = new File("student.txt"); // Create File object for "student.txt"

        Student s1 = new Student(); // Create two Students s1 and s2
        Student s2 = new Student();

        System.out.printf("Enter Student Name: \t"); // Get input from user Name & GPA for Student s1
        s1.setName(in.nextLine());
        System.out.printf("Enter Student GPA: \t");
        s1.setGPA(in.nextDouble());

        System.out.printf("Adding to %s \n", f.getName()); // Write s1 to the file.
        s1.Save(f);

        System.out.printf("Loading from %s \n", f.getName()); // Read into s2 from the file
        s2.Load(f);

        System.out.printf("Student: %s \n", s2); // print out student s2
    }
}
